...
class SigninWindow(BoxLayout):
    ...
    def validate_user(self):
        ...
        if uname == '' or passw == '':
            info.text = '[color=#FF0000] Usuario y contraseña necesarios[/color]'
        else:
            user = users.find_one({'user_name':uname})
            if user == None:
                info.text = '[color=#FF0000] Usuario o contraseña invalidos[/color]'
            else:
                passw = hashlib.sha256(passw.encode()).hexdigest()
                if passw == user['password']:
                    des = user['designation']
                    # info.text = '[color=#00FF00] Ingreso satisfactorio ADMIN!![/color]'
                    self.parent.parent.parent.ids.scrn_op.children[0].ids.loggedin_user.text = uname
                    if des == 'Administrador':
